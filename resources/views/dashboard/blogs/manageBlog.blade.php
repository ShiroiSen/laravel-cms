...
<div class="max-w-screen-2xl mx-auto mt-12">
    <div class=" flex flex-row mb-9 gap-5">
        ...
        <a href="/dashboard/blogs/trash" class="hover:text-blue-700 text-lg border border-black p-2 rounded-lg inline-block">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 inline-block">
                <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
            </svg> 
            <span class="align-middle inline-block text-sm">Trash Can</span>
        </a>
    </div>
    @if (session('success'))
    ...
    <div class="overflow-x-auto border border-black rounded-lg">
        <table class="w-full text-sm text-left rtl:text-right">
            <thead class="text-xs uppercase border-b-4 border-blue-600">
                <tr>
            ...
            <tbody>
                ...
                <tr class="hover:bg-gray-100 border">
                    <td scope="row" class="px-6 py-4 whitespace-nowrap">{{ $loop->iteration }}</td>
                    <td class="px-6 py-4 whitespace-nowrap">{{ $blog->title }}</td>
                    <td class="px-6 py-4 whitespace-nowrap">{{ $blog->category->name }}</td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        @if ($blog->is_confirmed === 'confirmed')
                            <p class="bg-green-200 text-green-400 py-1 px-2.5 rounded-full inline-flex items-center">
                        ...
                        @elseif ($blog->is_confirmed === 'rejected')
                            <p class="bg-red-200 text-red-400 py-1 px-2.5 rounded-full inline-flex items-center">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4 mr-1">
                    ...
                    <td class="px-6 py-4 flex flex-row justify-center gap-5 ">
                        ...
                        <form action="/dashboard/blogs/{{ $blog->slug }}" method="POST">
                            ...
                            {{-- <label for="is_trash" class="hidden"></label>
                            <input type="text" id="is_trash" name="is_trash" class="hidden" value="delete">
